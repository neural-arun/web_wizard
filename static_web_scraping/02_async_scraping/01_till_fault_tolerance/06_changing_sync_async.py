...
class BooksScraper:
    ...
    async def fetch_html(self, url: str) -> Optional[str]:
        # hame internet pr jaker wait karna hota hai aur hm wait ki jagah aur kaam karna chahate hain.
        timeout = aiohttp.ClientTimeout(total=10)  # timeout lagaye hain taaki agar server jyada time lagata hai to isko skip kar do.
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                # session open kiya aiohttp library ka use karke. 

                async with session.get(url) as response:
                    # yha se html uthaya jayega. yhi internet wait hota hai.  
                    logger.info(f"Status {response.status} for {url}")

                    if response.status == 200:
                        response.encoding = "utf-8"
                        return await response.text()
                        # html network se aata hai isiliye await.
                        ...
                        logger.error(f"[403] blocked by server")
                        return None
                        ...
                        logger.error(f"{response.status} unexpected response. {url}")
                        return None
        except asyncio.TimeoutError:
            logger.error(f"[Timeout] {url}")
            return None
        # aagr timeout error aaya to dekhega
        except aiohttp.ClientError as e:
            logger.error(f"[Network Error] {e} for {url}")
            return None
        # agar network error aaya to yaha dikhega.
    
    def parse_html(self,html: str) -> BeautifulSoup:
        return BeautifulSoup(html,"lxml")# it returns a BeautifulSoup object.
...
if __name__ == "__main__":
    ...
    logger.info("All book data saved to 'books_data.json'.")
